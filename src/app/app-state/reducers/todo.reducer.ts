...
const todoReducer = createReducer(
  ...
  on(todoActions.getTaskById, (state, { id }) => ({ ...state, isLoading: true, currentTask: id })),
  on(todoActions.getTaskByIdSuccess, (state, result) => {
    let tasks = undefined !== state.tasks ? _.cloneDeep(state.tasks) : [];
    if (result.status) {
      tasks = tasks.filter(task => task.id !== state.currentTask);
    }
    return {
      tasks,
      isLoading: false,
